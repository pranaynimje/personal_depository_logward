...
function SurchargePage({setPage,selectedLane,clearLane}){
  ...
  return (<div style={{padding:"20px 28px",width:"100%",boxSizing:"border-box"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:8}}>
      {[{side:"Origin",det:detO,dem:demO,dnd:dndO,detFP:BASE.costMatrix.detention_origin.avgFP+"d",demFP:BASE.costMatrix.demurrage_origin.avgFP+"d",dndFP:BASE.costMatrix.dnd_origin.avgFP+"d",dndFPNum:BASE.costMatrix.dnd_origin.avgFP},{side:"Dest",det:detD,dem:demD,dnd:dndD,detFP:BASE.costMatrix.detention_destination.avgFP+"d",demFP:BASE.costMatrix.demurrage_destination.avgFP+"d",dndFP:BASE.costMatrix.dnd_destination.avgFP+"d",dndFPNum:BASE.costMatrix.dnd_destination.avgFP}].map(s=>{const sepTotal=s.det+s.dem;const prem=s.dnd>sepTotal?Math.round((s.dnd-sepTotal)/sepTotal*100):0;const combinedSaves=assumedDays!==null?assumedDays<s.dndFPNum:null;const cheaper=assumedDays!==null?(combinedSaves?"Combined":"Separate"):(s.dnd>sepTotal?"Separate":"Combined");return <Card key={s.side}><div style={{fontSize:14,fontWeight:600,marginBottom:10}}>{"Combined vs Separate — "+s.side}</div>
