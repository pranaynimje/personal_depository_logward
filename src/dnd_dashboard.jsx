    ...
    <Card style={{padding:18,marginBottom:18,background:"#FEFCE808"}}>
      ...
      {[{label:"ORIGIN",bg:T.amberBg,lc:T.amber,nodes:[
        ...
        <div style={{background:sec.bg+"80",border:"none",borderRadius:14,padding:"16px 14px 12px",marginBottom:si===0?6:0,position:"relative"}}>
          ...
          <table style={{width:"100%",marginTop:10,borderCollapse:"collapse",fontSize:10}}><thead><tr style={{color:T.dim,fontSize:9}}><th style={{textAlign:"left",padding:"2px 6px",fontWeight:500}}>Type</th><th style={{textAlign:"right",padding:"2px 6px",fontWeight:500}}>Dwell</th><th style={{textAlign:"right",padding:"2px 6px",fontWeight:500}}>Free</th><th style={{textAlign:"right",padding:"2px 6px",fontWeight:500}}>Cost</th><th style={{textAlign:"right",padding:"2px 6px",fontWeight:500}}>#</th></tr></thead><tbody>{sec.metrics.map((m,i)=><tr key={i}><td style={{padding:"3px 6px",borderTop:"1px solid "+T.border+"30"}}><div style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:6,height:6,borderRadius:1,background:m.c}}/><span style={{fontWeight:500}}>{m.t}</span></div></td><td style={{padding:"3px 6px",textAlign:"right",fontWeight:600,color:m.dw>m.fp?T.red:T.text,borderTop:"1px solid "+T.border+"30"}}>{m.dw}d</td><td style={{padding:"3px 6px",textAlign:"right",color:T.dim,borderTop:"1px solid "+T.border+"30"}}>{m.fp}d</td><td style={{padding:"3px 6px",textAlign:"right",fontWeight:600,color:m.c,borderTop:"1px solid "+T.border+"30"}}>{fmt(m.cost)}</td><td style={{padding:"3px 6px",textAlign:"right",color:T.dim,borderTop:"1px solid "+T.border+"30"}}>{m.n}</td></tr>)}</tbody></table>
        </div>
      </div>)}
    </Card>
    <div style={{height:1,background:T.border+"40",margin:"6px 0 14px"}}/>
...
function CostPage({setPage}){
  ...
  return (<div style={{padding:"20px 28px",width:"100%",boxSizing:"border-box"}}>
    ...
    <Card>
      <div style={{fontSize:14,fontWeight:600,marginBottom:3}}>Full Cost Matrix</div><div style={{fontSize:11,color:T.sub,marginBottom:10}}>Detailed numbers with container counts and free periods for reporting</div>
      <table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 4px",fontSize:11}}>
        ...
        <tbody>{COST_CATS.map(cat=>{const o=cm[cat.oKey];const d=cm[cat.dKey];return <tr key={cat.name} style={{background:T.card2}}><td style={{padding:"7px 8px",borderRadius:"6px 0 0 6px"}}><div style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:8,height:8,borderRadius:2,background:cat.color}}/><span style={{fontWeight:700}}>{cat.name}</span></div></td><td style={{padding:"7px 8px",fontWeight:600,textAlign:"right"}}>{fmt(o.total)}</td><td style={{padding:"7px 8px",color:T.sub,textAlign:"right"}}>{o.withCost}</td><td style={{padding:"7px 8px",color:T.sub,textAlign:"right"}}>{o.avgFP}d</td><td style={{padding:"7px 8px",fontWeight:600,textAlign:"right"}}>{fmt(d.total)}</td><td style={{padding:"7px 8px",color:T.sub,textAlign:"right"}}>{d.withCost}</td><td style={{padding:"7px 8px",color:T.sub,textAlign:"right"}}>{d.avgFP}d</td><td style={{padding:"7px 8px",borderRadius:"0 6px 6px 0",color:cat.color,fontWeight:600,textAlign:"right"}}>{fmt(o.total+d.total)}</td></tr>;})}<tr><td colSpan={7} style={{padding:8,fontWeight:600,fontSize:13}}>TOTAL</td><td style={{padding:8,color:T.red,fontWeight:700,fontSize:15,textAlign:"right"}}>{fmt(BASE.grandTotal)}</td></tr></tbody>
      </table>
  ...
  return (<div style={{padding:"20px 28px",width:"100%",boxSizing:"border-box"}}>
    ...
    <div style={{marginTop:14}}>
      ...
      <Card style={{marginTop:8}}><table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 4px",fontSize:11}}><thead><tr style={{color:T.dim,fontSize:10,textAlign:"left"}}>{["Lane","Vol","O.Det","O.Dem","D.Dem","D.Det","Total","BFP","$/Container","Dwell Score"].map(h=><th key={h} style={{padding:"5px 7px",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px",textAlign:["Vol","$/Container","Dwell Score"].includes(h)?"right":"left"}}>{h}{h==="BFP"&&<HoverTip text={"Days Beyond Free Period. Total dwell minus 9.9d combined FP."}/>}{h==="Dwell Score"&&<HoverTip text={"High: total dwell > 15d. Med: > 10d. Low: ≤ 10d."}/>}</th>)}</tr></thead>
        <tbody>{lanes.map((l,i)=>{const rk=l.totalDwell>15?"High":l.totalDwell>10?"Med":"Low";const rc=rk==="High"?T.red:rk==="Med"?T.amber:T.green;const sFlag=(l.surchargePct||35)>35;return <tr key={i} style={{background:sFlag?T.redBg+"60":T.card2}}><td style={{padding:"7px",borderRadius:"6px 0 0 6px",fontWeight:600,fontFamily:"monospace",fontSize:10}}>{l.lane}{sFlag&&<span style={{fontSize:9,color:T.red,marginLeft:4}}>{"D&D>"+(l.surchargePct||35)+"%"}</span>}</td><td style={{padding:"7px",color:T.sub,textAlign:"right"}}>{l.containers}</td><td style={{padding:"7px",color:T.amber,fontWeight:700}}>{l.avgODet.toFixed(1)}d</td><td style={{padding:"7px",color:T.purple,fontWeight:700}}>{l.avgODem.toFixed(1)}d</td><td style={{padding:"7px",color:T.purple,fontWeight:700}}>{l.avgDDem.toFixed(1)}d</td><td style={{padding:"7px",color:T.red,fontWeight:700}}>{l.avgDDet.toFixed(1)}d</td><td style={{padding:"7px",fontWeight:600}}>{l.totalDwell.toFixed(1)}d</td><td style={{padding:"7px",color:l.beyondFP>0?T.red:T.green,fontWeight:600}}>{l.beyondFP}d</td><td style={{padding:"7px",fontWeight:700,textAlign:"right",color:l.costPerContainer>500?T.red:l.costPerContainer>200?T.amber:T.green}}>{"$"+l.costPerContainer}</td><td style={{padding:"7px",borderRadius:"0 6px 6px 0",textAlign:"right"}}><SolidBadge color={rc}>{rk}</SolidBadge></td></tr>;})}</tbody></table></Card>
    </div>
  </div>);
}
...
function SurchargePage({setPage}){
  ...
  return (<div style={{padding:"20px 28px",width:"100%",boxSizing:"border-box"}}>
    ...
    <Card style={{marginTop:14}}>
      <div style={{fontSize:14,fontWeight:600,marginBottom:3}}>Free Period Utilization</div><div style={{fontSize:11,color:T.sub,marginBottom:10}}>How much of your free period is consumed? {">"}90% = zero buffer, negotiate more days.</div>
