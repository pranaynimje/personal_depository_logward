...
const Badge=({children,color=T.blue})=><span style={{background:color+"18",color,padding:"3px 9px",borderRadius:12,fontSize:11,fontWeight:600,whiteSpace:"nowrap",border:"1px solid "+color+"25"}}>{children}</span>;
...
const SH=({title,sub})=><div style={{marginBottom:14}}><div style={{color:T.text,fontSize:15,fontWeight:700}}>{title}</div>{sub&&<div style={{color:T.sub,fontSize:11,marginTop:2}}>{sub}</div>}</div>;
const Insight=({text})=><div style={{background:T.blueBg,borderRadius:8,padding:"10px 14px",marginTop:8,borderLeft:"3px solid "+T.blue}}><div style={{fontSize:12,fontWeight:600,color:T.blue}}>{text}</div></div>;
const NavLink=({text,onClick})=><div onClick={onClick} style={{fontSize:12,color:T.blueL,fontWeight:600,cursor:"pointer",marginTop:6,display:"flex",alignItems:"center",gap:4}}>{text}<ArrowRight size={10}/></div>;
function ChartBox({title,sub,children,h=260,insight,nav}){return <Card style={{padding:16}}>{title&&<div style={{color:T.text,fontSize:13,fontWeight:700}}>{title}</div>}{sub&&<div style={{color:T.sub,fontSize:11,marginBottom:8}}>{sub}</div>}<div style={{height:h}}>{children}</div>{insight&&<Insight text={insight}/>}{nav}</Card>;}
...
function HomePage({setPage}){
  ...
  return (<div style={{padding:"16px 24px",width:"100%",boxSizing:"border-box"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:12,marginBottom:12}}>
      <Card style={{padding:"16px 20px"}}>
        ...
        <div style={{display:"flex",gap:16,marginTop:10}}>
          {[{l:"Origin",v:fmt(BASE.totalOriginCost),s:originPct+"% of total",c:T.amber},{l:"Destination",v:fmt(BASE.totalDestCost),s:(100-originPct)+"% of total",c:T.purple},{l:"Storage",v:fmt(cm.storage_origin.total+cm.storage_destination.total),s:storagePct+"% of total",c:T.green}].map(k=><div key={k.l} style={{borderLeft:"3px solid "+k.c,paddingLeft:8}}><div style={{fontSize:14,fontWeight:700,color:T.text}}>{k.v}</div><div style={{fontSize:10,color:T.sub}}>{k.l} | {k.s}</div></div>)}
          <div style={{borderLeft:"3px solid "+T.blue,paddingLeft:8}}><div style={{fontSize:14,fontWeight:700,color:T.text}}>{BASE.summary.inProgress.toLocaleString()}</div><div style={{fontSize:10,color:T.sub}}>Active | {BASE.summary.totalContainers.toLocaleString()} total</div></div>
        </div>
      </Card>
    ...
    <div style={{background:T.blueBg,borderRadius:10,padding:"10px 16px",marginBottom:14,borderLeft:"4px solid "+T.blue}}>
      <div style={{fontSize:12,color:T.blue,fontWeight:600}}>{fth.red+" containers breaching free time today, adding ~"+fmt(topBurn)+"/day. "+originPct+"% of your "+fmt(BASE.grandTotal)+" exposure is at origin. Scroll down to see which containers need action."}</div>
    </div>
    ...
    <Card style={{marginBottom:14,borderLeft:"4px solid "+T.red,background:T.redBg+"40"}}>
      ...
      .map((r,i)=><div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:i<3?"1px solid "+T.border+"80":"none"}}>
        ...
        <div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:T.text}}>{r.pattern}</div><div style={{fontSize:11,color:T.sub}}>{"Probable cause: "+r.cause+" | Impact: "+r.impact}</div><div style={{fontSize:11,color:r.c,fontWeight:600}}>{"Action: "+r.action}</div></div>
      </div>)}
    </Card>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:14}}>
      {(()=>{const detFP=cm.detention_origin.avgFP;const detAvg=sd.origin_detention.avg;const detUtil=Math.round(detAvg/detFP*100);const sepT=cm.detention_origin.total+cm.demurrage_origin.total;const combPrem=sepT>0?Math.round((cm.dnd_origin.total-sepT)/sepT*100):0;const wc=Object.entries(BASE.carriers).reduce((a,[n,d])=>d.avgODet>a[1].avgODet?[n,d]:a,["",{avgODet:0,containers:0,missingMilestones:0}]);const pavg=Object.values(BASE.carriers).reduce((s,d)=>s+d.avgODet*d.containers,0)/Math.max(1,Object.values(BASE.carriers).reduce((s,d)=>s+d.containers,0));return[
        ...
        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:2}}><ins.icon size={13} color={ins.color}/><span style={{fontSize:13,fontWeight:700}}>{ins.title}</span>{ins.nav&&<span style={{marginLeft:"auto",fontSize:9,color:T.blueL,fontWeight:600}}>{"View →"}</span>}</div>
        <div style={{paddingLeft:20,fontSize:11,color:T.sub}}>{"Because: "+ins.because}</div>
    ...
    <Card id="actionTable" urgency="action" style={{borderLeft:"4px solid "+T.blue}}>
      ...
      <div style={{fontSize:11,color:T.sub,marginBottom:10}}>Sorted by daily burn rate. Act on highest $/day first to maximize cost avoidance.</div>
      <table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 3px",fontSize:10}}>
        ...
        <tbody>{CDATA.topRisk.slice(0,5).map((c,i)=>{const daily=Math.round((c.cost3d-c.cost)/3);return <tr key={i} style={{background:i===0?T.blueBg:T.card2,borderLeft:c.risk>=75?"3px solid "+T.red:undefined}}>
          <td style={{padding:"5px 6px",borderRadius:"6px 0 0 6px",fontFamily:"monospace",fontSize:10,fontWeight:700,borderLeft:c.risk>=75?"3px solid "+T.red:undefined}}>{c.cn}</td>
...
function CostPage({setPage}){
  ...
  return (<div style={{padding:"16px 24px",width:"100%",boxSizing:"border-box"}}>
    ...
    <Card>
      <div style={{fontSize:14,fontWeight:700,marginBottom:2}}>Full Cost Matrix</div><div style={{fontSize:11,color:T.sub,marginBottom:10}}>Detailed numbers with container counts and free periods for reporting</div>
      <table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 4px",fontSize:11}}>
        <thead><tr style={{color:T.dim,fontSize:10,textAlign:"left"}}>{["Category","Origin $","O #","O FP","Dest $","D #","D FP","Total"].map(h=><th key={h} style={{padding:"5px 8px",fontWeight:700,textTransform:"uppercase",textAlign:h!=="Category"?"right":"left"}}>{h}</th>)}</tr></thead>
...
function OptimizerPage(){
  ...
  const ContainerCard=({c,dimmed})=><div style={{display:"flex",justifyContent:"space-between",padding:"5px 8px",borderRadius:6,marginBottom:2,background:"#fff",opacity:dimmed?.3:1,borderLeft:"3px solid "+catColor(c.cat)}}>
    <div><div style={{fontSize:10,fontWeight:700}}>{c.cn}</div><div style={{fontSize:9,color:T.sub}}>{c.ca+" | "+c.po+"→"+c.pd+" | "+c.fpStatus}</div></div>
  ...
  return (<div style={{padding:"16px 24px",width:"100%",boxSizing:"border-box"}}>
    ...
    <Card style={{marginTop:14}}>
      ...
      {(()=>{const detFP=BASE.costMatrix.detention_origin.avgFP;const detAvg=BASE.stageDays.origin_detention.avg;const detUtil=Math.round(detAvg/detFP*100);const combPct=Math.round(BASE.costMatrix.dnd_origin.total/BASE.grandTotal*100);const sepT=BASE.costMatrix.detention_origin.total+BASE.costMatrix.demurrage_origin.total;const combPrem=sepT>0?Math.round((BASE.costMatrix.dnd_origin.total-sepT)/sepT*100):0;const top3=allContainers.slice(0,3);const top3burn=top3.reduce((s,c)=>s+c.daily,0);const top3sav=top3.reduce((s,c)=>s+c.sav3d,0);const maxODet=Math.max(...top3.map(c=>c.oDet));return[{c:T.green,t:"Origin detention at "+detUtil+"% free-time utilization",b:"Avg "+detAvg+"d vs "+detFP+"d free. "+(detUtil>90?"Any delay triggers charges immediately.":"Some buffer remains but monitor closely."),a:"Coordinate with depot to ensure gate-in within "+Math.floor(detFP)+" days of pickup."},
        ...
        {c:T.red,t:"Top 3 containers burn "+fmt(top3burn)+"/day combined",b:"These have been in origin detention for "+Math.round(maxODet)+"+ days, deep in higher tier rates.",a:"Clearing within 3 days avoids "+fmt(top3sav)+" in additional charges."}]})().map((ins,i)=><div key={i} style={{background:T.card2,borderRadius:8,padding:10,marginBottom:6,borderLeft:"3px solid "+ins.c}}><div style={{fontSize:11,fontWeight:700,marginBottom:2}}>{ins.t}</div><div style={{fontSize:11,color:T.sub}}>{"Because: "+ins.b}</div><div style={{fontSize:11,color:ins.c,fontWeight:600}}>{"Action: "+ins.a}</div></div>)}
    </Card>
  </div>);
}
...
function HistoryPage({setPage}){
  ...
  return (<div style={{padding:"16px 24px",width:"100%",boxSizing:"border-box"}}>
    ...
    <Card style={{marginTop:14}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:2}}>Stage-Wise Cost Contribution</div><div style={{fontSize:11,color:T.sub,marginBottom:10}}>Which journey stages drive cost? Focus on "Over" rows and follow the prevention recommendation.</div>
      <table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 3px",fontSize:10}}>
    ...
    <Card style={{marginTop:14}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:2}}>Port Benchmarking</div><div style={{fontSize:11,color:T.sub,marginBottom:6}}>Compare port performance. Ports above the portfolio average line are underperforming.</div>
    ...
    <div style={{marginTop:14}}>
      ...
      <Card style={{marginTop:8}}><table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 3px",fontSize:11}}><thead><tr style={{color:T.dim,fontSize:10,textAlign:"left"}}>{["Lane","Vol","O.Det","O.Dem","D.Dem","D.Det","Total","BFP","$/Container","Dwell Score"].map(h=><th key={h} style={{padding:"5px 7px",fontWeight:700,textTransform:"uppercase",textAlign:["Vol","$/Container","Dwell Score"].includes(h)?"right":"left"}}>{h}{h==="BFP"&&<HoverTip text={"Days Beyond Free Period. Total dwell minus 9.9d combined FP."}/>}{h==="Dwell Score"&&<HoverTip text={"High: total dwell > 15d. Med: > 10d. Low: ≤ 10d."}/>}</th>)}</tr></thead>
        <tbody>{lanes.map((l,i)=>{const rk=l.totalDwell>15?"High":l.totalDwell>10?"Med":"Low";const rc=rk==="High"?T.red:rk==="Med"?T.amber:T.green;const sFlag=(l.surchargePct||35)>35;return <tr key={i} style={{background:sFlag?T.redBg+"60":T.card2}}><td style={{padding:"7px",borderRadius:"6px 0 0 6px",fontWeight:700,fontFamily:"monospace",fontSize:10}}>{l.lane}{sFlag&&<span style={{fontSize:9,color:T.red,marginLeft:4}}>{"D&D>"+(l.surchargePct||35)+"%"}</span>}</td><td style={{padding:"7px",color:T.sub,textAlign:"right"}}>{l.containers}</td><td style={{padding:"7px",color:T.amber,fontWeight:700}}>{l.avgODet.toFixed(1)}d</td><td style={{padding:"7px",color:T.purple,fontWeight:700}}>{l.avgODem.toFixed(1)}d</td><td style={{padding:"7px",color:T.purple,fontWeight:700}}>{l.avgDDem.toFixed(1)}d</td><td style={{padding:"7px",color:T.red,fontWeight:700}}>{l.avgDDet.toFixed(1)}d</td><td style={{padding:"7px",fontWeight:700}}>{l.totalDwell.toFixed(1)}d</td><td style={{padding:"7px",color:l.beyondFP>0?T.red:T.green,fontWeight:700}}>{l.beyondFP}d</td><td style={{padding:"7px",fontWeight:700,textAlign:"right",color:l.costPerContainer>500?T.red:l.costPerContainer>200?T.amber:T.green}}>{"$"+l.costPerContainer}</td><td style={{padding:"7px",borderRadius:"0 6px 6px 0",textAlign:"right"}}><SolidBadge color={rc}>{rk}</SolidBadge></td></tr>;})}</tbody></table></Card>
    </div>
  </div>);
}
...
function SurchargePage({setPage}){
  ...
  return (<div style={{padding:"16px 24px",width:"100%",boxSizing:"border-box"}}>
    ...
    <Card style={{marginTop:12}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:2}}>Detailed Surcharge Table</div><div style={{fontSize:11,color:T.sub,marginBottom:8}}>Per-container cost by surcharge type. Higher $/container = higher priority for action.</div>
      <table style={{width:"100%",borderCollapse:"separate",borderSpacing:"0 4px",fontSize:11}}><thead><tr style={{color:T.dim,fontSize:10,textAlign:"left"}}>{["Category","Side","Total","Containers","FP","$/Container","Reason"].map(h=><th key={h} style={{padding:"5px 8px",fontWeight:700,textTransform:"uppercase",textAlign:["Total","$/Container"].includes(h)?"right":"left"}}>{h}</th>)}</tr></thead>
    ...
    <Card style={{marginTop:14}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:2}}>Free Period Utilization</div><div style={{fontSize:11,color:T.sub,marginBottom:10}}>How much of your free period is consumed? {">"}90% = zero buffer, negotiate more days.</div>
      {[{label:"Origin Detention",avg:4.8,fp:5.1,color:T.amber},{label:"Origin Demurrage",avg:0.87,fp:3.1,color:T.purple},{label:"Dest Demurrage",avg:2.6,fp:3.0,color:T.green},{label:"Dest Detention",avg:5.51,fp:6.0,color:T.red}].map((r,i)=>{const pct=Math.round((r.avg/r.fp)*100);const capped=Math.min(100,pct);const over=pct>100;return <div key={i} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:T.sub}}>{r.label}</span><span style={{fontSize:12,fontWeight:700,color:over?T.red:pct>90?T.red:pct>70?T.amber:T.green}}>{r.avg+"d / "+r.fp+"d ("+pct+"%)"}{over&&" ⚠ OVER"}</span></div><div style={{height:7,background:T.card2,borderRadius:4,overflow:"hidden",border:"1px solid "+T.border,position:"relative"}}><div style={{height:"100%",width:capped+"%",background:over?T.red:pct>90?T.red:pct>70?T.amber:T.green,borderRadius:4}}/></div></div>;})}
